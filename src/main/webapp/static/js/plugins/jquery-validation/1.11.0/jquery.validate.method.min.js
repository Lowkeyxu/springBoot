...
jQuery.validator.addMethod("myDigits", function (c, b, a) {
    var d = new RegExp("^(?:[1-9]\\d*|0)(?:\\.\\d{1," + a + "})?$");
    return this.optional(b) || d.test(c)
}, "请输入正数且小数位数不大于{0}位");
jQuery.validator.addMethod("myNegativeDigits", function (c, b, a) {
    var d = new RegExp("^(-)?(?:[1-9]\\d*|0)(?:\\.\\d{1," + a + "})?$");
    return this.optional(b) || d.test(c)
}, "小数位数不大于{0}位");
...
};jQuery.validator.addMethod("fax", function (a, b) {
    var c = /^(\d{3,4})?[-]?\d{7,8}$/;
